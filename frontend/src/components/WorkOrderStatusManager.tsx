...
import { Plus, Pencil, Trash2, GripVertical, Lock, ClipboardList } from 'lucide-react';
...
export default function WorkOrderStatusManager() {
  ...
  const handleSave = async () => {
    ...
    try {
      setSaving(true);
      if (editingStatus) {
        await customApi('/api/v1/work-order-statuses/update', 'POST', {
          id: editingStatus.id,
        ...
        await customApi('/api/v1/work-order-statuses/create', 'POST', {
          value: formValue.trim(),
          label: formLabel.trim(),
          color: formColor,
          sort_order: formSortOrder,
  ...
  const handleDelete = async (status: StatusItem) => {
    ...
    try {
      await customApi('/api/v1/work-order-statuses/delete', 'POST', { id: status.id });
      ...
      <CardContent>
        ...
        <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
          <DialogContent className="max-w-md" dir="rtl">
            <DialogHeader>
              <DialogTitle>
                {editingStatus ? 'تعديل حالة أمر العمل' : 'إضافة حالة جديدة'}
              </DialogTitle>
            </DialogHeader>
