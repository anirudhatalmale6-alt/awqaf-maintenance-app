...
import { Plus, Pencil, Trash2, GripVertical, Lock, Palette } from 'lucide-react';
...
const COLOR_OPTIONS = [
  ...
  { value: 'bg-amber-100 text-amber-800', label: 'برتقالي' },
...
export default function DesignStatusManager() {
  ...
  const fetchStatuses = async () => {
    try {
      setLoading(true);
      const res = await customApi<StatusItem[]>('/api/v1/design-statuses/list', 'GET');
  ...
  const handleSave = async () => {
    ...
    try {
      setSaving(true);
      if (editingStatus) {
        await customApi('/api/v1/design-statuses/update', 'POST', {
          id: editingStatus.id,
        ...
        await customApi('/api/v1/design-statuses/create', 'POST', {
          value: formValue.trim(),
          label: formLabel.trim(),
          color: formColor,
          sort_order: formSortOrder,
  ...
  const handleDelete = async (status: StatusItem) => {
    ...
    try {
      await customApi('/api/v1/design-statuses/delete', 'POST', { id: status.id });
      ...
      <CardContent>
        ...
        <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
          <DialogContent className="max-w-md" dir="rtl">
            <DialogHeader>
              <DialogTitle>
                {editingStatus ? 'تعديل حالة التصميم' : 'إضافة حالة جديدة'}
              </DialogTitle>
            </DialogHeader>
            <div className="space-y-4 mt-2">
              ...
              <div className="space-y-2">
                ...
                <Input
                  placeholder="مثال: بانتظار المراجعة"
                  value={formLabel}
                    ...
                    {COLOR_OPTIONS.map((c) => (
                      <SelectItem key={c.value} value={c.value}>
                        <div className="flex items-center gap-2">
                          <span className={`px-2 py-0.5 rounded text-xs ${c.value}`}>
                            {c.label}
                          </span>
                        </div>
                      </SelectItem>
