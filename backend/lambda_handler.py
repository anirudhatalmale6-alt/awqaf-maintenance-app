...
def handle_backend_request_sync(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    ...
    if not services_initialized:
        try:
            # Use asyncio.run() which creates a fresh event loop, runs the coroutine,
            # and closes the loop. This is the safest approach for Lambda where we
            # don't have a persistent event loop.
            asyncio.run(initialize_services_once())
        except RuntimeError as e:
            # If asyncio.run() fails (e.g., nested event loop), try alternative approaches
            logger.warning(f"asyncio.run() failed: {e}, trying alternative initialization")
            try:
                loop = asyncio.get_event_loop()
                if loop.is_running():
                    # If a loop is already running (unlikely in Lambda but possible),
                    # use nest_asyncio pattern or skip and let lazy init handle it
                    logger.warning("Event loop already running, deferring to lazy initialization")
                else:
                ...
                try:
                    ...
                    # Don't close the loop - Mangum may need it
                    pass
        except Exception as e:
            logger.error(f"Failed to initialize services in sync handler: {e}\n{format_traceback()}")
            return {
                "statusCode": 503,
                "headers": {"Content-Type": "application/json", "Access-Control-Allow-Origin": "*"},
                "body": json.dumps({"error": "Service temporarily unavailable", "message": "Database initialization in progress, please retry"}),
            }
