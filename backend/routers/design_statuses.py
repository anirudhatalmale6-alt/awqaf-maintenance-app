...
from models.design_statuses import Design_statuses

logger = logging.getLogger(__name__)

router = APIRouter(prefix="/api/v1/design-statuses", tags=["design-statuses"])
...
DEFAULT_STATUSES = [
    {"value": "draft", "label": "مسودة", "color": "bg-gray-100 text-gray-800", "sort_order": 1, "is_default": True},
    {"value": "in_review", "label": "قيد المراجعة", "color": "bg-amber-100 text-amber-800", "sort_order": 2, "is_default": True},
    {"value": "approved", "label": "معتمد", "color": "bg-green-100 text-green-800", "sort_order": 3, "is_default": True},
    {"value": "rejected", "label": "مرفوض", "color": "bg-red-100 text-red-800", "sort_order": 4, "is_default": True},
]


async def ensure_default_statuses(db: AsyncSession):
    count_query = select(func.count(Design_statuses.id))
    result = await db.execute(count_query)
    count = result.scalar() or 0

    if count == 0:
        for s in DEFAULT_STATUSES:
            status = Design_statuses(**s)
            db.add(status)
        await db.commit()
        logger.info("Default design statuses created")
...
async def list_statuses(db: AsyncSession = Depends(get_db)):
    ...
    query = select(Design_statuses).order_by(
        Design_statuses.sort_order.asc(), Design_statuses.id.asc()
    )
...
async def create_status(
    ...
    existing_query = select(Design_statuses).where(Design_statuses.value == data.value.strip())
    existing_result = await db.execute(existing_query)
    if existing_result.scalar_one_or_none():
        raise HTTPException(status_code=400, detail="قيمة الحالة موجودة بالفعل")

    new_status = Design_statuses(
...
async def update_status(
    ...
    query = select(Design_statuses).where(Design_statuses.id == data.id)
    result = await db.execute(query)
    ...
    if data.value is not None and data.value.strip():
        ...
        dup_query = select(Design_statuses).where(
            Design_statuses.value == data.value.strip(),
            Design_statuses.id != data.id,
        )
